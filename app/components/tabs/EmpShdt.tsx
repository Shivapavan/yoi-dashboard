...
interface Employee {
  employee: string; totalHours: number; shifts: Shift[]
}
interface Data {
  startDate: string; endDate: string
  employees: Employee[]; totalHours: number
}
...
const DEFAULT_HOURLY_RATE = 10  // dollars per hour
// Per-employee overrides (case-insensitive match on the employee name)
const EMPLOYEE_RATES: Record<string, number> = {
  janu: 10.5,
}
function rateFor(employee: string): number {
  return EMPLOYEE_RATES[employee.trim().toLowerCase()] ?? DEFAULT_HOURLY_RATE
}
function pay(hours: number, employee: string) {
  const rate = rateFor(employee)
  return `$${(hours * rate).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
}
function payAmount(hours: number, employee: string): number {
  return hours * rateFor(employee)
}
...
// Generate all Mondays from Aug 4 2025 to the current week
...
export default function EmpShdt() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const r = await fetch(`/api/emp-shifts?view=weekly&param=${weekStart}`)
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
            ...
            <div className="bg-white rounded-lg p-4 shadow-sm border-l-4 border-green-500">
              <div className="text-xs text-gray-500 uppercase tracking-wide mb-1">Avg Hours</div>
              <div className="text-xl font-bold text-gray-900">
                {data.employees.length ? hm(data.totalHours / data.employees.length) : '—'}
              </div>
            </div>
          </div>
                            ...
                            <table className="w-full text-xs">
                              <thead>
                                <tr className="text-gray-400">
                                  ...
                                  <th className="text-right py-1 pr-4">Hours</th>
