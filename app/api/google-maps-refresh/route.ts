...
import { scrapeGoogleMapsProfiles, type PlaceResult } from '@/lib/scraper/google-maps'

const RUN_STATE_BLOB = 'google-maps-run-state.json'
...
async function doScrape() {
  ...
  if (!apiKey) { await writeState({ status: 'failed', startedAt: new Date().toISOString(), error: 'Missing GOOGLE_PLACES_API_KEY' }); return }

  try {
    const profiles = await scrapeGoogleMapsProfiles(BUSINESSES, apiKey)
    const intel = buildIntel(profiles)
    const blob = await put('google-maps-intel.json', JSON.stringify(intel), { access: 'public', contentType: 'application/json', addRandomSuffix: false, allowOverwrite: true })
...
function buildIntel(profiles: PlaceResult[]) {
  const sorted = [...profiles].sort((a, b) => b.rating - a.rating)
  return {
    lastUpdated: new Date().toISOString(),
    totalPlaces: profiles.length,
    places: sorted,
